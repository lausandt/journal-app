...
async def update_user(*, id: int, update: UpdatePassword) -> UserOutSchema:  # type: ignore
    update.password = get_password_hash(update.password)
    await User.filter(id=id).update(**update.model_dump(exclude_unset=True))
    return await UserOutSchema.from_queryset_single(User.get(id=id))

async def set_super(id:int):
    await User.filter(id=id).update(superuser=True)
    return await UserDatabaseSchema.from_queryset_single(User.get(id=id))


async def delete_user(id: int) -> Status:
